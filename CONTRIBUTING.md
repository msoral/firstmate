...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, and `.agents/skills/`.
  ...
  A local `config/backend` file explicitly overrides runtime auto-detection for new task endpoints and stays gitignored; accepted values are the default `zellij`, the still-selectable `tmux`, and experimental `herdr`.
  It does not make `data/` tracked.
- Helper scripts in `bin/` are plain bash.
  Each starts with a usage header comment; keep it accurate when you change behavior.
  Test scripts and helpers in `tests/` are plain bash too.
  `shellcheck bin/*.sh bin/backends/*.sh tests/*.sh` must pass, and CI enforces it.
- Changes to harness adapters (detection in `bin/fm-harness.sh`, launch and hook mechanics in `bin/fm-spawn.sh`, busy signatures in `bin/fm-watch.sh`, `bin/fm-tmux-lib.sh`, and `bin/fm-zellij-lib.sh`, cleanup in `bin/fm-teardown.sh`, and facts in `.agents/skills/harness-adapters/SKILL.md`) must be verified empirically against the real harness, never written from documentation alone.
- Changes to runtime session backends (`bin/fm-backend.sh`, `bin/backends/`, and the scripts that dispatch through them) need empirical adapter notes in the relevant docs: the default reference backend's notes live in `docs/zellij-backend.md`, and any new backend still gets its own `docs/<name>-backend.md` empirical doc (as `docs/herdr-backend.md` does for herdr).
...
tests/fm-session-start.test.sh            # fm-session-start.sh: ABSENT vs empty-vs-present digest files, lock-refusal read-only path skipping every mutating step, diagnostics-first section ordering, status-tail bounding, tmux/zellij/herdr endpoint liveness, and composition of the real fm-lock/fm-bootstrap/fm-wake-drain scripts
...
tests/fm-backend-zellij.test.sh           # fake zellij CLI unit tests for the default zellij adapter and fm-zellij-lib.sh: version/jq gates, named-pane create, target parsing, send/capture, current-path via list-panes --json, dump-screen composer detection, and verified submit retry
tests/fm-backend-zellij-smoke.test.sh     # real zellij smoke test for the default zellij adapter, skipped unless real zellij >=0.44.0, jq, and python3 are available: named-pane create, send, bounded capture, current-path, live-pane resolve, kill
