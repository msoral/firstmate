...
On the detached native-background path (claude, grok) the foreground launcher persists the resolved pane so the daemon does not fall back to `firstmate:0`, and the daemon proves that pane injectable once at away-mode entry so a wrong pane alarms within seconds instead of after max-defer (`docs/afk-inject-delivery.md`).
