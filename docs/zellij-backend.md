...
| Headless session start | `zellij attach -b <name>` with stdin redirected from `/dev/null` and no controlling TTY | Creates the session and returns promptly (cannot actually attach without a TTY, so it exits after creating). The session persists with zero attached clients - `dump-screen`, `list-panes`, etc. all work against it. Running it again against an EXISTING live session prints `"Session already exists"` and exits 1 - harmless, since existence is checked first via `list-sessions` and the launch call's own exit status is never inspected. Against a stopped-but-resurrectable EXITED husk, `attach -b` instead RESURRECTS it (this is how `server_ensure` rebuilds a dead session without destroying operator state) - see "Dead-session detection" below. |
