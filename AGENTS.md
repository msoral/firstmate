...
config/backend  runtime session-provider backend override for new tasks; LOCAL, gitignored; absent = falls through to runtime auto-detection (the runtime firstmate itself is executing inside), then zellij; zellij is the default reference backend (docs/zellij-backend.md, needs zellij >= 0.44.0 + jq), tmux is still fully selectable, herdr is the experimental backend (docs/herdr-backend.md); not inherited into secondmate homes
...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, worktree=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=; kind=secondmate also records home= and projects=; a task on a non-tmux runtime backend also records backend= (absent still means tmux, for legacy compatibility; the default zellij backend records backend=zellij plus zellij_session=/zellij_pane_id= - docs/zellij-backend.md; the experimental herdr backend records herdr_session=, herdr_workspace_id=, herdr_tab_id=, herdr_pane_id= too - docs/herdr-backend.md; bin/fm-backend.sh, section 8) (fm-pr-check, including through fm-pr-merge, appends pr= and GitHub's pr_head= when available; fm-x-link appends x_request= and x_request_ts= for an X-mention-originated task, section 14)
...
For the zellij backend (the default), each task is a single pane labeled `fm-<id>`, and the recorded `window=` target is `<zellij-session>:<pane-id>` (e.g. `firstmate:terminal_5`).
...
4. Use the `window=` values from the digest's `state/*.meta` entries as the live direct-report set, and read the digest's per-task `endpoint: alive|dead` line for each - that cheap check is already done; do not re-probe it yourself.
   Do not sweep every `fm-*` zellij pane, tmux window, or herdr tab across all sessions during recovery; another firstmate home's child endpoints may share that namespace and are not this home's orphans.
...
All truth lives in each task's backend live-task inventory (zellij by default, or tmux/herdr when selected or auto-detected), state files, data/backlog.md, data/captain.md, data/learnings.md, data/secondmates.md, persistent secondmate homes, and treehouse; your conversation memory is a cache.
...
bin/fm-spawn.sh <id> projects/<repo> --backend zellij  # explicit runtime backend; zellij is the default reference backend (docs/zellij-backend.md), version-gates at spawn
bin/fm-spawn.sh <id> projects/<repo> --backend tmux   # tmux backend (still fully selectable)
...
The script resolves the harness (`fm-harness.sh crew` for crewmate/scout tasks only when `config/crew-dispatch.json` is absent, `fm-harness.sh secondmate` for `kind=secondmate`; section 4), resolves the runtime backend (`--backend`, then `FM_BACKEND`, then `config/backend`, then runtime auto-detection - the runtime firstmate itself is executing inside, from `$TMUX`/`$ZELLIJ`/`HERDR_ENV=1` markers, nesting resolved innermost-first - then the default `zellij`; an auto-detected herdr spawn prints a loud stderr notice, auto-detected zellij or tmux stays silent), owns the verified launch templates, resolves the project's delivery mode (`fm-project-mode.sh`) for ship/scout tasks, and records `harness=`, `model=`, `effort=`, `kind=`, `mode=`, and `yolo=` in the task's meta; a non-tmux runtime backend is recorded as `backend=` (a zellij task also records zellij_session=/zellij_pane_id=), and a tmux task omits backend= because absent means tmux.
...
For ship and scout tasks, the script creates the runtime endpoint (a zellij pane by default, a tmux window when `backend=tmux`, or a herdr tab/pane when `backend=herdr`), runs `treehouse get`, waits for the worktree subshell, asserts the resolved worktree is a genuine isolated worktree distinct from the primary checkout (aborting the spawn otherwise, to prevent the worktree tangle of section 8), installs the turn-end hook, records `state/<id>.meta`, and launches the agent with the brief.
...
Each task's backend live-task inventory is the ground truth (zellij by default; absent `backend=` means tmux for legacy tasks; a task's meta records `backend=` for any non-tmux backend - zellij is the default reference backend, docs/zellij-backend.md, and herdr is the experimental one, docs/herdr-backend.md).
